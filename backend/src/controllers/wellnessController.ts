...
// Add wellness data
export const addWellnessData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await checkAndGenerateAlerts(userId as string, wellnessEntry);
...
export const getDashboardData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const stats = calculateWellnessStats(wellnessEntries, (req as AuthRequest).user.preferences.goals);
...
export const getWellnessHistory = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
...
export const createWellnessGoal = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
...
// Get wellness goals
export const getWellnessGoals = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
...
const calculateWellnessStats = (entries: any[], goals: any) => {
  ...
  const averageSteps = Math.round(entries.reduce((sum, entry) => sum + (entry.steps || 0), 0) / entries.length);
  const averageSleep = Math.round((entries.reduce((sum, entry) => sum + (entry.sleepHours || 0), 0) / entries.length) * 10) / 10;
  const averageStudy = Math.round((entries.reduce((sum, entry) => sum + (entry.studyHours || 0), 0) / entries.length) * 10) / 10;
  const averageWater = Math.round((entries.reduce((sum, entry) => sum + (entry.diet?.waterIntake || 0), 0) / entries.length) * 10) / 10;
  ...
  const stats = {
    ...
    goalProgress: {
      steps: Math.round((averageSteps / goals.dailySteps) * 100),
      sleep: Math.round((averageSleep / goals.sleepHours) * 100),
      study: Math.round((averageStudy / goals.studyHours) * 100),
      water: Math.round((averageWater / goals.waterIntake) * 100)
    }
  };
