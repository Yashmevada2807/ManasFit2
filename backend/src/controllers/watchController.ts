...
// Connect Fitbit
export const connectFitbit = async (req: Request, res: Response): Promise<void> => {
  try {
...
// Connect Google Fit
export const connectGoogleFit = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
...
export const syncWatchData = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
    ...
    switch (provider) {
      case 'fitbit':
        syncResult = await syncFitbitData(connection, userId as string);
        break;
      case 'google-fit':
        syncResult = await syncGoogleFitData(connection, userId as string);
        break;
      default:
        res.status(400).json({
          success: false,
...
export const disconnectWatch = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
    const { provider } = req.body;
