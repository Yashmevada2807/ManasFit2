...
User's preferences: ${JSON.stringify((req as AuthRequest).user.preferences)}
...
export const getWellnessInsights = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as AuthRequest).user._id;
    ...
    const recommendations = generateRecommendations(entries, (req as AuthRequest).user.preferences);
